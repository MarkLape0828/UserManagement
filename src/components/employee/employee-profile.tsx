...
export function EmployeeProfile({ user }: EmployeeProfileProps) {
  const getInitials = (firstName: string, lastName: string) => {
    ...
    return (firstInitial + lastInitial).toUpperCase() || 'U'; // Fallback to 'U' if no initials
  };
      ...
      <Card className="w-full max-w-2xl shadow-xl overflow-hidden">
        ...
        <CardContent className="p-6 sm:p-8 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <InfoItem icon={<UserCircle className="text-primary" />} label="Full Name" value={fullName} />
